...
export default async function EmbedProductPricePage() {
  const { session, permissions } = await requirePagePermission("set_product_price");
  const user = session.user;

  const rows = await getProducts();

  return (
    <DashboardShell user={{ name: user.name, role: user.role, permissions }} titleKey="sidebar.productPrices">

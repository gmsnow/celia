...
export default async function ShowExpensesPage() {
  const { session, permissions } = await requirePagePermission("view_expenses");
  const user = session.user;

  const summary = await getExpenses();

  return (
    <DashboardShell user={{ name: user.name, role: user.role, permissions }} titleKey="sidebar.viewExpenses">
